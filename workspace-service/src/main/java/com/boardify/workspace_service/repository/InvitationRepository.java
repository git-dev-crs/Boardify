...
public interface InvitationRepository extends JpaRepository<InvitationModel, InvitationModelId> {
    @Query("SELECT i FROM InvitationModel i WHERE i.id = :id")
    Optional<InvitationModel> findInvitationById(@Param("id") InvitationModelId id);
    ...
    @Query("""
            ...
            WHERE im.id.workspaceId = :workspaceId
            """)
